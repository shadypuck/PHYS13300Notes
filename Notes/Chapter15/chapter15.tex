...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item Let $\mu_1\ll\mu_2$, i.e., one string is tied to an immovable wall. Then $A_r/A_i\to -1$ and $A_t/A_i\to 0$, as we would expect by Newton's third law.
        \item Let $\mu_1\gg\mu_2$. Then $A_r/A_i\to 1$ and $A_t/A_i\to 2$.
    \end{itemize}
    ...
    \begin{itemize}
        ...
        \end{enumerate}
        \item From the second boundary condition, we know that we must have $\sin(kL)=0$, i.e., $kL=n\pi$ for some $n\in\N$ (the wavenumber cannot be negative or zero by definition).
        \item Thus, $k_n=\frac{n\pi}{L}$.
        \item It follows since $k=\frac{2\pi}{\lambda}$ that $L=\frac{n}{2}\cdot\lambda$.
    \end{itemize}
    \item More specifically, if $L=\frac{n}{2}\cdot\lambda$ for some $n\in\N$, then we will have a \textbf{standing wave}.
...
\begin{itemize}
    \item \marginnote{8/10:}An alternate way of deriving the speed of a harmonic wave:
    \begin{itemize}
